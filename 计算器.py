...
class Calc(tk.Frame):  # 创建Calc类继承Frame:框架控件；在屏幕上显示一个矩形区域，多用来作为容器
    def __init__(self, parent=None):
        ...
        self.expr.set('')
        self.exprLabel = tk.Label(self, font=('Helvetica', 10), fg='#666666', width=64, anchor='e', textvariable=self. expr)
        self.exprLabel.grid(row=0, column=0, columnspan=4)
        self.result = tk.StringVar()  # 显示结果
        self.result.set(0)
        self.resultLabel = tk.Label(self, font=('Helvetica', 20), width=34, anchor='e', textvariable=self.result)
